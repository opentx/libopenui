...
class DynamicText: public StaticText
{
  public:
    DynamicText(Window * parent, const rect_t & rect, std::function<std::string()> textHandler, LcdFlags textFlags = 0):
      StaticText(parent, rect, textHandler ? textHandler() : "", 0, textFlags),
...
class DynamicNumber: public Window
{
  public:
    ...
    void paint(BitmapBuffer * dc) override
    {
      dc->drawNumber((textFlags & RIGHT) ? width() : 0, FIELD_PADDING_TOP, value, PRIMARY_COLOR, textFlags, 0, prefix, suffix);
    }
